...
const verify = `const fs=require('fs');const path=require('path');const root=process.cwd();let ok=true;for(const rel of ${JSON.stringify(targets)}){const full=path.join(root,rel);if(!fs.existsSync(full)){console.error('[missing]',rel);ok=false;continue}const text=fs.readFileSync(full,'utf8');for(const token of ['containsInternalSignals','containsPersonalSignals','containsBusinessSignals','containsSecretSignals','PrivacyGateDecisionObject','decision: PrivacyGateDecisionObject','recommendedAction: string','safePayloadPreview','providerDispatchAllowed','networkCallAllowed','finalDispatchBlocked']){if(!text.includes(token)){console.error('[missing token]',rel,token);ok=false}else console.log('[ok token]',rel,token)}}if(ok)console.log('[OK] privacy gate detected flags verify passed');process.exit(ok?0:1);`;
write('scripts/v-fix-privacy-gate-detected-flags.cjs', verify);

const pkgPath = path.join(root, 'package.json');
const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
pkg.scripts = pkg.scripts || {};
pkg.scripts['fixprivacygateflags:verify'] = 'node scripts/v-fix-privacy-gate-detected-flags.cjs';
fs.writeFileSync(pkgPath, JSON.stringify(pkg, null, 2) + '\n', 'utf8');
console.log('[write] package.json script fixprivacygateflags:verify');
console.log('[OK] privacy gate detected flags fix applied');
