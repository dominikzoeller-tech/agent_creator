...
const verify = `const fs=require('fs');const path=require('path');const root=process.cwd();let ok=true;for(const rel of ['frontend/lib/cmt-master-committee-status.ts','frontend/app/lib/cmt-master-committee-status.ts']){const full=path.join(root,rel);if(!fs.existsSync(full)){console.error('[missing]',rel);ok=false;continue}const text=fs.readFileSync(full,'utf8');for(const token of ['testPrompts: string[]','defaultTestPrompts','testPrompts: defaultTestPrompts','roles: string[]','externalSharingAllowed']){if(!text.includes(token)){console.error('[missing token]',rel,token);ok=false}else console.log('[ok token]',rel,token)}}if(ok)console.log('[OK] committee status testPrompts field verify passed');process.exit(ok?0:1);`;
write('scripts/v-fix-committee-status-testprompts-field.cjs', verify);

const pkgPath = path.join(root, 'package.json');
const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
pkg.scripts = pkg.scripts || {};
pkg.scripts['fixcommitteetestprompts:verify'] = 'node scripts/v-fix-committee-status-testprompts-field.cjs';
fs.writeFileSync(pkgPath, JSON.stringify(pkg, null, 2) + '\n', 'utf8');
console.log('[write] package.json script fixcommitteetestprompts:verify');
console.log('[OK] committee status testPrompts field fix applied');
