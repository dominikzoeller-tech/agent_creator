...
function createOutcome(option: PrivacyDecisionOption, detected: PrivacyGateDetected): PrivacyDecisionOutcome {
  if (option === 'cancel') return { accepted: false, mode: option, message: 'Vorgang wurde abgebrochen.', nextAction: 'Keine externe Weitergabe ausfuehren.' };
  ...
  if (option === 'anonymize_then_send') return { accepted: true, mode: option, message: 'Anonymisierung ist erforderlich. Externe Weitergabe bleibt bis zur Freigabe blockiert.', nextAction: 'Anonymisierte Vorschau pruefen.' };
  return {
...
const verify = `const fs=require('fs');const path=require('path');const root=process.cwd();let ok=true;for(const rel of ${JSON.stringify(targets)}){const full=path.join(root,rel);if(!fs.existsSync(full)){console.error('[missing]',rel);ok=false;continue}const text=fs.readFileSync(full,'utf8');for(const token of ['providerDispatchAllowed: false','networkCallAllowed: false','finalDispatchBlocked: true','safePayloadPreview: string','safePayloadPreview: sanitizedText','PrivacyDecisionOutcome','outcome: PrivacyDecisionOutcome','PrivacyDecisionGate','decidePrivacyAction']){if(!text.includes(token)){console.error('[missing token]',rel,token);ok=false}else console.log('[ok token]',rel,token)}}if(ok)console.log('[OK] privacy decision dispatch flags verify passed');process.exit(ok?0:1);`;
write('scripts/v-fix-privacy-decision-dispatch-flags.cjs', verify);

const pkgPath = path.join(root, 'package.json');
const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
pkg.scripts = pkg.scripts || {};
pkg.scripts['fixprivacydispatchflags:verify'] = 'node scripts/v-fix-privacy-decision-dispatch-flags.cjs';
fs.writeFileSync(pkgPath, JSON.stringify(pkg, null, 2) + '\n', 'utf8');
console.log('[write] package.json script fixprivacydispatchflags:verify');
console.log('[OK] privacy decision dispatch flags fix applied');
