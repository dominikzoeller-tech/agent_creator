...
export function simulateProviderRequestEnvelopePolicy(input:{ providerRequestEnvelopeId?: string; metadata?: Record<string, unknown> }): ProviderRequestEnvelopePolicySimulation {
  ...
  checks.push({ name:"network_provider_blocked", passed:envelope?.networkCallPerformed===false && envelope?.providerExecutionAllowed===false, reason:"Netzwerk-/Provider-AusfÃ¼hrung bleibt blockiert." });
