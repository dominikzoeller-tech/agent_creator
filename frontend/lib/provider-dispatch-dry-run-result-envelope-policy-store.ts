...
export function simulateProviderDispatchDryRunResultEnvelopePolicy(input:{ providerDispatchDryRunResultEnvelopeId?: string; metadata?: Record<string, unknown> }): ProviderDispatchDryRunResultEnvelopePolicySimulation {
  ...
  let reason="Provider Dispatch Dry-Run Result Envelope Policy erlaubt nur no-provider-call Simulation. Result Envelope enthÃ¤lt keine Provider Response.";
  ...
  else if(envelope.resultEnvelopeContainsProviderResponse!==false || envelope.providerResponseIncluded!==false || envelope.providerResultIncluded!==false){ decision="blocked_result_envelope_contains_provider_response"; reason="Result Envelope enthÃ¤lt Provider Response oder Provider Result."; }
