...
export function createProviderAgnosticLlmInvocationAdapterStub(input:{ invocationEnvelopeId?: string; metadata?: Record<string, unknown> }): ProviderAgnosticLlmInvocationAdapterStub {
  ...
  checks.push({ name:"execution_blocked", passed: env?.executionAllowed === false && env?.toolExecutionAllowed === false && env?.agentExecutionAllowed === false && env?.invocationEnvelope?.toolExecutionAllowed === false && env?.invocationEnvelope?.agentExecutionAllowed === false, reason:"Execution-, Tool- und Agent-AusfÃ¼hrung bleiben blockiert." });
