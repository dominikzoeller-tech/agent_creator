...
export function simulateProviderDispatchTranscriptEnvelopePolicy(input:{ providerDispatchTranscriptEnvelopeId?: string; metadata?: Record<string, unknown> }): ProviderDispatchTranscriptEnvelopePolicySimulation {
  ...
  checks.push({ name:"payloads_not_included", passed:envelope?.dispatchPayloadIncluded===false && envelope?.commandPayloadIncluded===false && envelope?.requestBodyIncluded===false && envelope?.sensitiveRequestBodyIncluded===false, reason:"Dispatch-, Command- und Request-Payloads dÃ¼rfen nicht enthalten sein." });
  ...
  let reason="Provider Dispatch Transcript Envelope Policy erlaubt nur no-provider-call Simulation. Transcript enthÃ¤lt keine Provider Response, keinen Prompt Payload und keine Secrets.";
  ...
  else if(envelope.transcriptEnvelopeContainsProviderResponse!==false || envelope.providerResponseIncluded!==false || envelope.providerResultIncluded!==false){ decision="blocked_transcript_contains_provider_response"; reason="Transcript enthÃ¤lt Provider Response oder Provider Result."; }
  else if(envelope.transcriptEnvelopeContainsPromptPayload!==false || envelope.promptPayloadIncluded!==false || envelope.promptIncluded!==false){ decision="blocked_transcript_contains_prompt_payload"; reason="Transcript enthÃ¤lt Prompt Payload."; }
  else if(envelope.transcriptEnvelopeContainsSecrets!==false || envelope.secretValuesIncluded!==false || envelope.noSecretsIncluded!==true || containsSecretValue(envelope)){ decision="blocked_transcript_contains_secrets"; reason="Transcript enthÃ¤lt Secret-Werte."; }
  else if(envelope.resultEnvelopeContainsProviderResponse!==false){ decision="blocked_result_envelope_contains_provider_response"; reason="Result Envelope enthÃ¤lt Provider Response."; }
