...
export function createProviderDispatchReleaseCandidateEnvelope(input:{ providerDispatchTranscriptEnvelopeId?: string; metadata?: Record<string, unknown> }): ProviderDispatchReleaseCandidateEnvelope {
  ...
  let reason="Provider Dispatch Release Candidate Envelope wurde nur vorbereitet und ist nur bereit fÃ¼r Human Review. Es wird nicht approved, nicht ausgefÃ¼hrt und enthÃ¤lt keine Provider Response, keinen Prompt Payload und keine Secrets. Kein Provider-/Netzwerk-Aufruf.";
  ...
  else if(transcript.transcriptEnvelopeContainsProviderResponse!==false || transcript.providerResponseIncluded!==false || transcript.providerResultIncluded!==false){ decision="blocked_transcript_contains_provider_response"; reason="Transcript enthÃ¤lt Provider Response oder Provider Result."; }
  else if(transcript.transcriptEnvelopeContainsPromptPayload!==false || transcript.promptPayloadIncluded!==false || transcript.promptIncluded!==false){ decision="blocked_transcript_contains_prompt_payload"; reason="Transcript enthÃ¤lt Prompt Payload."; }
  else if(transcript.transcriptEnvelopeContainsSecrets!==false || transcript.secretValuesIncluded!==false || transcript.noSecretsIncluded!==true || containsSecretValue(transcript)){ decision="blocked_transcript_contains_secrets"; reason="Transcript enthÃ¤lt Secret-Werte."; }
  else if(transcript.resultEnvelopeContainsProviderResponse!==false){ decision="blocked_result_envelope_contains_provider_response"; reason="Result Envelope enthÃ¤lt Provider Response."; }
  else if(transcript.commandEnvelopeExecuted!==false){ decision="blocked_command_envelope_executed"; reason="Command Envelope wurde ausgefÃ¼hrt."; }
  ...
  else if(transcript.providerDispatchPerformed!==false){ decision="blocked_dispatch_performed"; reason="Provider Dispatch wurde ausgefÃ¼hrt oder ist nicht sicher false."; }
  ...
  else if(transcript.networkCallAllowed!==false || transcript.networkCallPerformed!==false || transcript.providerExecutionAllowed!==false){ decision="blocked_network_or_provider_execution_attempt"; reason="Netzwerk-/Provider-AusfÃ¼hrung erkannt."; }
