...
export function simulateProviderAdapterPolicy(input:{ adapterStubId?: string; metadata?: Record<string, unknown> }): ProviderAdapterPolicySimulation {
  ...
  checks.push({ name:"provider_none", passed: stub?.providerRequestPreview?.provider === "none" && stub?.providerRequestPreview?.modelSelected === "none", reason:"Kein Provider und kein Modell dÃ¼rfen ausgewÃ¤hlt sein." });
  ...
  checks.push({ name:"execution_blocked", passed: stub?.executionAllowed === false && stub?.toolExecutionAllowed === false && stub?.agentExecutionAllowed === false, reason:"Execution-, Tool- und Agent-AusfÃ¼hrung mÃ¼ssen blockiert bleiben." });
  ...
  else if(stub.networkCallPerformed !== false || stub.providerRequestPreview?.networkCallAllowed !== false){ decision="blocked_network_call_detected"; reason="Netzwerk-/Provider-Aufruf wÃ¤re erlaubt oder wurde durchgefÃ¼hrt."; }
