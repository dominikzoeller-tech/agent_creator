...
export function createProviderDispatchTranscriptEnvelope(input:{ providerDispatchDryRunResultEnvelopeId?: string; metadata?: Record<string, unknown> }): ProviderDispatchTranscriptEnvelope {
  ...
  let reason="Provider Dispatch Transcript Envelope wurde nur vorbereitet. Es enthÃ¤lt keine Provider Response, keinen Prompt Payload und keine Secrets. Kein Provider-/Netzwerk-Aufruf.";
  ...
  else if(resultEnvelope.resultEnvelopeContainsProviderResponse!==false || resultEnvelope.providerResponseIncluded!==false || resultEnvelope.providerResultIncluded!==false){ decision="blocked_result_envelope_contains_provider_response"; reason="Result Envelope enthÃ¤lt Provider Response oder Provider Result."; }
  else if(resultEnvelope.commandEnvelopeExecuted!==false){ decision="blocked_command_envelope_executed"; reason="Command Envelope wurde ausgefÃ¼hrt."; }
  ...
  else if(resultEnvelope.providerDispatchPerformed!==false){ decision="blocked_dispatch_performed"; reason="Provider Dispatch wurde ausgefÃ¼hrt oder ist nicht sicher false."; }
  ...
  else if(resultEnvelope.networkCallAllowed!==false || resultEnvelope.networkCallPerformed!==false || resultEnvelope.providerExecutionAllowed!==false){ decision="blocked_network_or_provider_execution_attempt"; reason="Netzwerk-/Provider-AusfÃ¼hrung erkannt."; }
