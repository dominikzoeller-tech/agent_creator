...
export function simulateControlledProviderSimulationPolicy(input:{ simulationEnvelopeId?: string; metadata?: Record<string, unknown> }): ControlledProviderSimulationPolicySimulation {
  ...
  checks.push({ name:"provider_none", passed:request.provider === "none" && request.modelSelected === "none", reason:"Provider und Modell mÃ¼ssen none bleiben." });
  checks.push({ name:"network_blocked", passed:request.networkCallAllowed === false && envelope?.networkCallPerformed === false && envelope?.providerExecutionAllowed === false, reason:"Netzwerk-/Provider-AusfÃ¼hrung muss blockiert bleiben." });
  checks.push({ name:"prompt_and_secrets_excluded", passed:request.promptIncluded === false && request.secretValuesIncluded === false && envelope?.noSecretsIncluded === true && !containsSecretValue(envelope), reason:"Prompt und Secret-Werte dÃ¼rfen nicht enthalten sein." });
  ...
  checks.push({ name:"execution_blocked", passed:envelope?.executionAllowed === false && envelope?.toolExecutionAllowed === false && envelope?.agentExecutionAllowed === false, reason:"Execution-, Tool- und Agent-AusfÃ¼hrung mÃ¼ssen blockiert bleiben." });
