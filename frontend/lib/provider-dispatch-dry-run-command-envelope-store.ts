...
export function createProviderDispatchDryRunCommandEnvelope(input:{ providerDispatchExecutionGateId?: string; metadata?: Record<string, unknown> }): ProviderDispatchDryRunCommandEnvelope {
  ...
  let reason="Provider Dispatch Dry-Run Command Envelope wurde nur vorbereitet. Command Envelope wird nicht ausgefÃ¼hrt. Kein Provider-/Netzwerk-Aufruf.";
  ...
  else if(gate.providerDispatchPerformed!==false){ decision="blocked_dispatch_performed"; reason="Provider Dispatch wurde bereits ausgefÃ¼hrt oder ist nicht sicher false."; }
  ...
  else if(gate.networkCallAllowed!==false || gate.networkCallPerformed!==false || gate.providerExecutionAllowed!==false){ decision="blocked_network_or_provider_execution_attempt"; reason="Netzwerk-/Provider-AusfÃ¼hrung erkannt."; }
