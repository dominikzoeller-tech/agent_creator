/* Legacy CMT compatibility module: cmt-intake.ts. */
...
export function selectCommitteeRoles(topic: CommitteeQuestion['topic'], riskLevel: CommitteeQuestion['riskLevel']): CommitteeRoleId[] {
  ...
  return committeeRoles.filter((role: any) => base.includes(role.id)).map((role: any) => role.id);
}
...
export default makeCompatStub('default:cmt-intake.ts');
