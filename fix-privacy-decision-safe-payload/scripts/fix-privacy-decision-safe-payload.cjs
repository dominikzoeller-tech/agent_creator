...
const verify = `const fs=require('fs');const path=require('path');const root=process.cwd();let ok=true;for(const rel of ${JSON.stringify(targets)}){const full=path.join(root,rel);if(!fs.existsSync(full)){console.error('[missing]',rel);ok=false;continue}const text=fs.readFileSync(full,'utf8');for(const token of ['safePayloadPreview: string','safePayloadPreview: sanitizedText','PrivacyDecisionOutcome','outcome: PrivacyDecisionOutcome','PrivacyDecisionGate','PrivacyGateDecisionObject','decidePrivacyAction']){if(!text.includes(token)){console.error('[missing token]',rel,token);ok=false}else console.log('[ok token]',rel,token)}}if(ok)console.log('[OK] privacy decision safe payload verify passed');process.exit(ok?0:1);`;
write('scripts/v-fix-privacy-decision-safe-payload.cjs', verify);

const pkgPath = path.join(root, 'package.json');
const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
pkg.scripts = pkg.scripts || {};
pkg.scripts['fixprivacysafepayload:verify'] = 'node scripts/v-fix-privacy-decision-safe-payload.cjs';
fs.writeFileSync(pkgPath, JSON.stringify(pkg, null, 2) + '\n', 'utf8');
console.log('[write] package.json script fixprivacysafepayload:verify');
console.log('[OK] privacy decision safe payload fix applied');
