...
function patchPackage(){
  ...
  pkg.scripts["phase37:1:patch"]="node scripts/phase37-1-patch-provider-dispatch-dry-run-command-envelope-policy-audit.cjs";
  pkg.scripts["phase37:1:verify"]="node scripts/phase37-1-verify-provider-dispatch-dry-run-command-envelope-policy-audit.cjs";
  pkg.scripts["llm:provider-dispatch-dry-run-command-envelope:policy:verify"]="node scripts/phase37-1-verify-provider-dispatch-dry-run-command-envelope-policy-audit.cjs";
  write(file, JSON.stringify(pkg,null,2)+"\n");
  console.log("OK package.json: Phase 37.1 Scripts eingetragen.");
}
...
`;
const api=`import { listProviderDispatchDryRunCommandEnvelopePolicySimulations, simulateProviderDispatchDryRunCommandEnvelopePolicy, summarizeProviderDispatchDryRunCommandEnvelopePolicySimulations } from "../../../lib/provider-dispatch-dry-run-command-envelope-policy-store";
export const dynamic = "force-dynamic";
export async function GET(request: Request){ try{ const url=new URL(request.url); const limit=Number(url.searchParams.get("limit")||"100"); const simulations=listProviderDispatchDryRunCommandEnvelopePolicySimulations(Number.isFinite(limit)?limit:100); return Response.json({ ok:true, summary:summarizeProviderDispatchDryRunCommandEnvelopePolicySimulations(simulations), simulations }); } catch(error){ const message=error instanceof Error ? error.message : "Provider Dispatch Dry-Run Command Envelope Policy Simulations konnten nicht gelesen werden."; return Response.json({ ok:false, error:message }, { status:500 }); } }
export async function POST(request: Request){ try{ const body=await request.json(); const simulation=simulateProviderDispatchDryRunCommandEnvelopePolicy({ providerDispatchDryRunCommandEnvelopeId: typeof body.providerDispatchDryRunCommandEnvelopeId==="string" ? body.providerDispatchDryRunCommandEnvelopeId : undefined, metadata: body.metadata && typeof body.metadata==="object" ? body.metadata : undefined }); return Response.json({ ok:true, simulation }); } catch(error){ const message=error instanceof Error ? error.message : "Provider Dispatch Dry-Run Command Envelope Policy Simulation konnte nicht erstellt werden."; return Response.json({ ok:false, error:message }, { status:400 }); } }
`;
...
export default function ProviderDispatchDryRunCommandEnvelopePolicyPage(){
 ...
 async function load(){ setError(null); try{ const [eRes,sRes]=await Promise.all([fetch("/api/provider-dispatch-dry-run-command-envelope?limit=100",{cache:"no-store"}),fetch("/api/provider-dispatch-dry-run-command-envelope-policy?limit=100",{cache:"no-store"})]); const e=await eRes.json(); const s=await sRes.json(); if(eRes.ok){ const list=Array.isArray(e.providerDispatchDryRunCommandEnvelopes)?e.providerDispatchDryRunCommandEnvelopes:[]; setEnvelopes(list); if(!selected && list[0]?.id) setSelected(list[0].id); } if(!sRes.ok) throw new Error(s?.error||"Policy Simulations konnten nicht geladen werden."); setSims(Array.isArray(s.simulations)?s.simulations:[]); setSummary(s.summary||null); }catch(err){ setError(err instanceof Error?err.message:"Unbekannter Fehler"); } }
 useEffect(()=>{ load(); },[]);
 async function simulate(){ const res=await fetch("/api/provider-dispatch-dry-run-command-envelope-policy",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({providerDispatchDryRunCommandEnvelopeId:selected})}); if(!res.ok){ const p=await res.json(); setError(p?.error||"Policy Simulation fehlgeschlagen"); } await load(); }
 return <main className="page-wrap"><UnifiedNavigation active="provider-dispatch-dry-run-command-envelope-policy" /><div className="page-shell"><section className="hero-card" style={{background:"linear-gradient(135deg,#f0fdf4 0%,#f8fafc 100%)",borderColor:"#86efac"}}><h1 className="section-title">Provider Dispatch Dry-Run Command Envelope Policy</h1><p style={{lineHeight:1.6}}>Phase 37.1 simuliert Policy Checks für Provider Dispatch Dry-Run Command Envelopes. Command Envelope bleibt nicht ausgeführt. Kein Provider Dispatch und kein Provider-/Netzwerk-Aufruf.</p></section>{error ? <section className="panel-card" style={{borderColor:"#fecaca",background:"#fef2f2"}}>{error}</section> : null}<section className="panel-card"><h2>Policy Simulation erstellen</h2><select className="text-input" value={selected} onChange={(ev)=>setSelected(ev.target.value)}>{envelopes.map((item)=><option key={item.id} value={item.id}>{item.envelopeMode} · {item.decision} · {item.id}</option>)}</select><button className="primary-button" type="button" onClick={simulate} disabled={!selected}>Provider Dispatch Dry-Run Command Envelope Policy simulieren</button></section><section className="panel-card"><h2>Summary</h2><pre style={{whiteSpace:"pre-wrap"}}>{JSON.stringify(summary??{},null,2)}</pre></section><section className="panel-card"><h2>Policy Simulations</h2>{sims.length===0?<p>Noch keine Policy Simulations.</p>:sims.map((sim)=><article key={sim.id} style={{borderTop:"1px solid #e5e7eb",padding:"12px 0"}}><div><strong>{sim.policyMode}</strong> <span className="chip">{sim.decision}</span></div><div className="helper-text"><code>{sim.id}</code> · {sim.timestamp}</div><p><strong>Reason:</strong> {sim.reason}</p><p><strong>Prepared:</strong> {String(sim.providerDispatchDryRunCommandEnvelopePrepared)} · <strong>Command prepared:</strong> {String(sim.commandEnvelopePrepared)} · <strong>Command executed:</strong> {String(sim.commandEnvelopeExecuted)} · <strong>Gate open:</strong> {String(sim.executionGateOpen)}</p><p><strong>Final dispatch allowed:</strong> {String(sim.finalDispatchAllowed)} · <strong>Network allowed:</strong> {String(sim.networkCallAllowed)} · <strong>Network call:</strong> {String(sim.networkCallPerformed)} · <strong>Provider execution:</strong> {String(sim.providerExecutionAllowed)} · <strong>LLM Call:</strong> {String(sim.llmCallPerformed)} · <strong>Dry-run:</strong> {String(sim.dryRunOnly)}</p><ul>{sim.policyChecks?.map((check)=><li key={check.name}><strong>{check.name}:</strong> {String(check.passed)} – {check.reason}</li>)}</ul></article>)}</section></div></main>;
}
`;
function patchNavigation(){ const file="frontend/components/UnifiedNavigation.tsx"; if(!exists(file)) return console.log("SKIP UnifiedNavigation fehlt."); let content=read(file); if(content.includes('provider-dispatch-dry-run-command-envelope-policy')){ console.log("SKIP UnifiedNavigation: Dry-Run Command Envelope Policy bereits vorhanden."); return; } const line='  { href: "/provider-dispatch-dry-run-command-envelope-policy", label: "Dispatch Dry-Run Policy", key: "provider-dispatch-dry-run-command-envelope-policy" },'; const markers=['{ href: "/provider-dispatch-dry-run-command-envelope", label: "Dispatch Dry-Run Command", key: "provider-dispatch-dry-run-command-envelope" },']; let patched=false; for(const marker of markers){ if(content.includes(marker)){ content=content.replace(marker, marker+'\n'+line); patched=true; break; } } if(!patched){ const idx=content.indexOf("];",content.indexOf("href:")); if(idx<0) throw new Error("Konnte Navigation-Liste nicht finden."); content=content.slice(0,idx)+line+'\n'+content.slice(idx); } write(file, content); console.log("OK UnifiedNavigation: Dispatch Dry-Run Policy Link ergänzt."); }
function patchDocs(){ ensureFile("phase37-1-provider-dispatch-dry-run-command-envelope-policy-audit.md", `# Phase 37.1 – Provider Dispatch Dry-Run Command Envelope Policy & Audit\n\n## Ziel\nProvider Dispatch Dry-Run Command Envelope wird per Policy Simulation geprüft und als Governance Audit Event protokolliert.\n\n## UI/API\n- UI: /provider-dispatch-dry-run-command-envelope-policy\n- API: /api/provider-dispatch-dry-run-command-envelope-policy\n\n## Sicherheitsinvarianten\n- providerDispatchDryRunCommandEnvelopePrepared=true\n- commandEnvelopePrepared=true\n- commandEnvelopeExecuted=false\n- executionGateOpen=false\n- finalDispatchAllowed=false\n- providerDispatchPerformed=false\n- networkCallAllowed=false\n- networkCallPerformed=false\n- providerExecutionAllowed=false\n- realLlmCallAllowed=false\n- llmCallPerformed=false\n- dryRunOnly=true\n\n## Nächster Schritt\nPhase 37.2 – Provider Dispatch Dry-Run Command Envelope Dashboard & Smoke\n`); ensureFile("docs/phase37-provider-dispatch-dry-run-command-envelope-policy-audit-runbook.md", `# Runbook – Phase 37.1 Provider Dispatch Dry-Run Command Envelope Policy & Audit\n\n## Patch\n\`\`\`powershell\nnpm run phase37:1:patch\n\`\`\`\n\n## Verify\n\`\`\`powershell\nnpm run phase37:1:verify\nnpm run build\n\`\`\`\n`); }
patchPackage();
ensureFile("frontend/lib/provider-dispatch-dry-run-command-envelope-policy-store.ts", store);
ensureFile("frontend/app/api/provider-dispatch-dry-run-command-envelope-policy/route.ts", api);
ensureFile("frontend/app/provider-dispatch-dry-run-command-envelope-policy/page.tsx", page);
patchNavigation();
patchDocs();
console.log("Phase 37.1 Patch abgeschlossen.");
